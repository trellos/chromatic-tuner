...
function updateTunaField(centsValue: number | null, isDetecting: boolean): void {
  ...
  const hasPitch = isDetecting && centsValue !== null;
  const boundedCents = hasPitch ? Math.max(-50, Math.min(50, centsValue)) : 0;
  const detune = hasPitch ? Math.abs(boundedCents) / 50 : 1;
  const direction = hasPitch && boundedCents !== 0 ? Math.sign(boundedCents) : 0;
  const sharpness = hasPitch ? Math.max(0, boundedCents) / 50 : 0;
  const flatness = hasPitch ? Math.max(0, -boundedCents) / 50 : 0;
  ...
  tunaFieldEl.classList.toggle("is-detecting", hasPitch);
  tunaFieldEl.classList.toggle("is-sharp", hasPitch && direction > 0);
  tunaFieldEl.classList.toggle("is-flat", hasPitch && direction < 0);
  ...
  tunaTiles.forEach((tile, index) => {
    ...
    tile.style.opacity = `${(hasPitch ? 0.45 + detune * 0.22 : 0.22).toFixed(3)}`;
  });
}
