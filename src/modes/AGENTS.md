...
- Bass **style** (riff pattern, `BASS_STYLES`) and bass **sound** (synth voice) are independent dropdowns.
- Bass **sound** = 6 voices ported from the sibling `OutrunAxe` repo's `EddieBass` (Battle / Cliff Diver), variants `option-1`..`option-6` (Pluck Bass / Acid Growl / Staccato / Deep Rumble / Techno Stab / Slap Bass). Shared architecture: sine sub + detuned bite layer → shared lowpass with a fast cutoff envelope (peak→floor) = the pluck "bite"; they differ in wave/filter/decay, and `option-6` adds an octave layer + noise transient. The `BASS_VOICES` table lives atop `playBassNote`; keep in sync with OutrunAxe.
