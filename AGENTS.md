...
- Bass lines are built across the whole progression in `resolveProgression`/`buildBassLine`, so each bar's final sounding eighth targets the following chord (wrapping from the last bar back to bar 1). The 14 styles span relentless root pumps, octave pumps, the Texas box, boogie walks, a unison blues riff, a pentatonic climb, two-feel/half-time grooves, syncopated pushes, and slower blues-rock figures (slow shuffle, boogie drone, busy walking blues, turnaround lick, rolling shuffle, half-time swagger).
- **Bass sound = 6 selectable voices ported from the sibling `OutrunAxe` repo's `EddieBass` (used by its Battle and Cliff Diver modes), variants `option-1`..`option-6`.** Picked via the **Bass Sound** dropdown; default `option-1`. The dropdown labels map: `option-1` Pluck Bass, `option-2` Acid Growl, `option-3` Staccato, `option-4` Deep Rumble, `option-5` Techno Stab, `option-6` Slap Bass. Every voice shares one architecture — a sine **sub** (body) + a detuned **bite** layer (edge) summed into a shared lowpass whose cutoff snaps from a peak down to a floor on the attack (that fast filter envelope is the pluck "bite") — and differs in wave blend, filter shape, and decay; `option-6` adds an octave-up layer and a finger-pop noise transient. The voice table (`BASS_VOICES`) lives in `src/modes/blues-jam.ts`; keep the params in sync if the OutrunAxe `EddieBass` voices are retuned. **Bass style** (riff pattern) and **bass sound** (synth voice) are independent.
...
- Controls: 12-key radiogroup, tempo slider (60–160 BPM), progression `<select>`, **bass-style `<select>`** (the riff pattern), **bass-sound `<select>`** (the synth voice), **Bass / Chord volume sliders**, a `Chords` checkbox, and a Play/Stop button. All selections persist to `localStorage` (`tuna.bluesJam.*`).
...
- **Bass styles (`BassStyleId`, `BASS_STYLES`):** `root-pump`, `octave-pump`, `box`, `boogie`, `blues-riff`, `pentatonic`, `two-feel`, `syncopated-push`, `slow-shuffle`, `boogie-drone`, `walking-blues`, `turnaround-lick`, `rolling-shuffle`, `half-time-swagger`. Each is an 8-slot eighth-note grid generated per bar by `buildBassLine(styleId, seed, nextRoot)` (`null` = rest); the last sounding slot leads chromatically into the next bar's root. Each style declares a `feel` (`straight`/`shuffle`/`half`) that drives off-beat placement and the drum pattern — adding a style with a different feel needs no scheduler change.
